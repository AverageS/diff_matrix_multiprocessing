...
if __name__ == '__main__':
    ...
    S = list(range(SUBSTITUTION_LENGTH))
    random.shuffle(S)
    s_len = len(S)
    ...
    print("--- WORKERS_COUNT: %d, BLOCK_SIZE: %d, SUBSTITUTION_LENGTH: %d    takes %s seconds ---"
          % (WORKERS_COUNT, BLOCK_SIZE, SUBSTITUTION_LENGTH, time.time() - start_time))

    start_time = time.time()
    P_one = one_proc(S)
    print("--- one process: %s seconds ---" % (time.time() - start_time))
